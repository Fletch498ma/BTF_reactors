...
CFGFILE_EK2="/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/config_ir_ek2.cfg"
CFGFILE_EK3="/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/config_ec_ek3.cfg"
...
logdir='/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021'
...
def mklogfile(sensorname,rowType,parm,phidget_present=False):
    ...
    if parm == 'zero':
        logpath = path.join(logdir,f"Zero/{sensorname}/{gettimestamp(timeformat=True)}.csv")#path.join(logdir,f"{reactor}/{sensorname}/{stream}/{gettimestamp()}.csv")
    elif parm == 'spanCH4':
        logpath = path.join(logdir,f"SpanCH4/{sensorname}/{gettimestamp(timeformat=True)}.csv")#path.join(logdir,f"{reactor}/{sensorname}/{stream}/{gettimestamp()}.csv")
    elif parm == 'spanCO2':
        logpath = path.join(logdir,f"SpanCO2/{sensorname}/{gettimestamp(timeformat=True)}.csv")#path.join(logdir,f"{reactor}/{sensorname}/{stream}/{gettimestamp()}.csv")
...
def main(e):
    ...
    storage = {'N2':['C','flow','N2','zero',0.5,{'ir_ek2':[None,None],'ec_ek3':[None,None],'humidity':[None,None],'gas_pressure':[None,None],'alicat_bench':[None,None]}],
                '60%CH4 in 40%CO2':['A','flow','Bio60M','spanCO2',5,{'ir_ek2':[None,None],'ec_ek3':[None,None],'humidity':[None,None],'gas_pressure':[None,None],'alicat_bench':[None,None]}],
                '2%CH4 in N2':['B','flow',255,'spanCH4',50,{'ir_ek2':[None,None],'ec_ek3':[None,None],'humidity':[None,None],'gas_pressure':[None,None],'alicat_bench':[None,None]}]
                }
...
#    
    ...
    for gas,parms in storage.items():
        if gas == 'N2':
            port=cfg.get('alicat_bench','port')
            timer=300
            mfc1=storage['60%CH4 in 40%CO2']
            ...
            if sensors_present.get(curSensor):
                ...
                logFile = mklogfile('alicat',rowType,parms[3])
            ...
            if sensors_present.get(curSensor):
                ...
                logFile = mklogfile('alicat',rowType,parms[3])
                sensors[curSensor] = Alicat(commConfig=commConfig,sensorName=curSensor,AlicatCache=dfAlicatData,threadName=curSensor,eventType='wait_event',event=e,rowType=rowType,rateSec=1,avg=10,timer=timer,db=None,log=logFile)
                sensors[curSensor].mass_flow = 50
                sensors[curSensor].delimiterPattern = "\\S+"
                sensors[curSensor].delimiter = " "
                
                commConfig = {'port':cfg.get(curSensor,'port'),'address':mfc1[0],'control_point':mfc1[1],'gas':mfc1[2]}
                rowType = (('gas',str),('setpoint',int),('mass_flow',int),('pressure',int),('temperature',float))
                logFile = mklogfile('alicat',rowType,parms[3])
                sensors[curSensor] = Alicat(commConfig=commConfig,sensorName=curSensor,AlicatCache=dfAlicatData,threadName=curSensor,eventType='wait_event',event=e,rowType=rowType,rateSec=1,avg=10,timer=timer,db=None,log=logFile)
                sensors[curSensor].mass_flow = 0.45
                sensors[curSensor].delimiterPattern = "\\S+"
                sensors[curSensor].delimiter = " "
        ...
        if sensors_present.get(curSensor):
            ...
            logFile = mklogfile(curSensor,rowType,parms[3],phidget_present=True)
        ...
        for name,sensor in sensors.items():
            ...
            if name == 'alicat_bench':
                if gas == 'N2':
                    ...
                    ir_querie('set A concentration range 1',value=60000)#ppm
                    ir_querie('set B concentration range 1',value=40000)
    ...
    #                    IRdata.insert(7,'ChB Compensated Span (Old Zero)',(IRdata['ChB Compensated Noramlised Absorbance (Old Zero)']/(1-exp(-a_coef * (3 ** c_coef))))+(beta*((IRdata['Temperature (C)'] - Temp_cal)/Temp_cal))) #2 is CH4 calibration gas concentration in %v/
                    ...
                    parser2={'spanPres':['ek2 co2 span pressure','pressure',SpanCO2_Pres],
                             'spanPresstd':['ek2 co2 span pressure','std',SpanCO2_Pres_std]}
                    parser3={'rangePresEK3':['ek3 max range pressure','pressure',SpanCO2_Pres],
                             'rangePresstdEK3':['ek3 max range pressure','std',SpanCO2_Pres_std]}
                    
                    cfgLog(CFGFILE_EK2,parser2)
                    cfgLog(CFGFILE_EK3,parser3)
                    ...
                    parser2={'spanHum':['ek2 co2 span humidity','humidity',SpanCO2_Hum],
                            'spanHumstd':['ek2 co2 span humidity','std',SpanCO2_Hum_std]}
                    parser3={'rangeHumEK3':['ek3 max range humidity','humidity',SpanCO2_Hum],
    ...
    zeroIRdir = '/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/Zero/ir_ek2/OldParameters.csv'
    ...
    rowZeroEC = (('TimeIndex',float),('Date',str),('Time',str),('Zero O2',float),('Zero O2 STD',float),
                ('Temperature (C)',float),('Temp STD',float),('Pressure (kPa)',float),('Pressure STD',float),('Humidity (%)',float),
                ('Humidity STD',float))
    zeroECdir = '/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/Zero/ec_ek3/OldParameters.csv'
    ...
    SpanCO2dir = '/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/SpanCO2/ir_ek2/OldParameters.csv'
    ...
    rowSpanCH4 = (('TimeIndex',float),('Date',str),('Time',str),('Span CH4',float),('Span CH4 STD',float),('Span CH4 EK2',float),
                  ('Temperature (C)',float),('Temp STD',float),('Pressure (kPa)',float),('Pressure STD',float),('Humidity (%)',float),('Humidity STD',float))
    SpanCH4dir = '/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/SpanCH4/ir_ek2/OldParameters.csv'
    ...
    rowSpanO2 = (('TimeIndex',float),('Date',str),('Time',str),('Span O2',float),('Span O2 STD',float),
                  ('Temperature (C)',float),('Temp STD',float),('Pressure (kPa)',float),('Pressure STD',float),('Humidity (%)',float),('Humidity STD',float))
    SpanO2dir = '/media/pi/One Touch/Seagate/raspberrypi/Biotrickling_filter_data/log2021/SpanO2/ec_ek3/OldParameters.csv'
