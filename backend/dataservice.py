...
class SensorReader(Thread):
    ...
    def __init__(self,commConfig,sensorName,rowType = (),rateSec=10,avg=10,db=None,log=None):
        ...
        self.rowType = (("time",float),) + rowType
        self.log = log#.name
        #log.close()
        self.end = False
    ...
    def run(self): 
        while(not self.end):
            if localtime().tm_sec%self.rateSec == 0:
                ...
                now = datetime.now()
                timeStamp = [self.sensorName,now.strftime("%d-%b-%Y")]
                logEntry = timeStamp + avgRows[1:]
                #logEntry = ",".join(
                        #["%s"%repr(avgRows)]) + "\n" 
                print(self.log)
                if self.log:
...
class SerialSensorReader(SensorReader):
    def __init__(self,commConfig,sensorName,rowType=(),rateSec=10,avg=10,db=None,log=None):
        SensorReader.__init__(self,commConfig,sensorName,rowType,rateSec,avg,db,log)
        self.eol = "\n"
        self.delimiterPattern = "\\S+"
        self.delimiter = " "
    def initSensor(self):
...
class  IRSensorReader(SerialSensorReader):
    def __init__(self,commConfig,sensorName,rowType=(),rateSec=10,avg=10,db=None,log=None):
        SerialSensorReader.__init__(self,commConfig,sensorName,rowType,rateSec,avg,db,log)
        self.eol = '\n\r'
...
class  ECSensorReader(SerialSensorReader):
    ...
    def avgRows(self, readings):
        ...
        for r in readings: #make sure sensor reading values are all the same type, indicated by the rowType[1]
            ...
            for t,c in zip(list(zip(*self.rowType))[1],r):
                indx += 1
                if indx == 2 or indx == 3 or indx == 4 or indx == 5 or indx == 6: #columns with hexadecimal base
                    try:
                        typed.append(t(c,16))
                    except ValueError:
                        print("Conversion error in line:",repr(r))
