...
def MakePlot():
    ...
    for Group in Experiments:
        for Experiment in Group:
            Query = "SELECT EXTRACT (EPOCH FROM metonetime) AS metonetime,metone03 FROM metone WHERE	metonetime >= (SELECT starttime - interval '10 minutes' FROM experiments WHERE experiment = {0}) AND metonetime < (SELECT endtime + interval '10 minutes' FROM experiments WHERE experiment = {0}) ORDER BY	metonetime".format(Experiment)
            ...
            for Row in Rows:
                if TimeCounter == 0:
                    ...
                    x.append(x[len(x)-1] + Row[0] - LastTime)
                y.append(Row[1])
                LastTime = Row[0]
                TimeCounter += 1
        ...
        P.ylabel('Counts / L')
        P.grid(True)
        P.savefig("{}_Metone03.png".format(Count))
        #P.show()
        P.close()
        Count += 1
#HTMLSummary()
