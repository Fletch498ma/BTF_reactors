...
for i in range(0,4):
    ...
    xraw0 = pd.to_datetime(OutletFileIR['Date'],format='%Y-%m-%d')
    if i == 2:
        yOutCH4 = OutletFileIR['CH4 (%v/v)'][22:len(OutletFileIR)]
        yOutCO2 = OutletFileIR['CO2 (%v/v)'][22:len(OutletFileIR)]
        xOutvalIR = [index for index in range(0,len(yOutCH4))]
    elif i == 3:
        yOutCH4 = OutletFileIR['CH4 (%v/v)'][21:len(OutletFileIR)]
        yOutCO2 = OutletFileIR['CO2 (%v/v)'][21:len(OutletFileIR)]
        xOutvalIR = [index for index in range(0,len(yOutCH4))]
    else:
...
#   xOutvalIR = np.arange(rangex.days)
    ...
    xraw1 = pd.to_datetime(InletFileIR['Date'],format='%Y-%m-%d')
    if i == 2:
        yInCH4 = InletFileIR['CH4 (%v/v)'][16:len(InletFileIR)]
        yInCO2 = InletFileIR['CO2 (%v/v)'][16:len(InletFileIR)]
        xInvalIR = [index for index in range(0,len(yInCH4))]
    elif i == 3:
        yInCH4 = InletFileIR['CH4 (%v/v)'][15:len(InletFileIR)]
        yInCO2 = InletFileIR['CO2 (%v/v)'][15:len(InletFileIR)]
        xInvalIR = [index for index in range(0,len(yInCH4))]
    else:
    ...
    xraw2 = pd.to_datetime(OutletFileO2['Date'],format='%Y-%m-%d')
    xOutO2 = [index for index in range(10,len(xraw2))]
    #if i == 0:
    ...
    xraw3 = pd.to_datetime(InletFileO2['Date'],format='%Y-%m-%d')
    xInO2 = [index for index in range(10,len(xraw3))]
    #if i == 0:
